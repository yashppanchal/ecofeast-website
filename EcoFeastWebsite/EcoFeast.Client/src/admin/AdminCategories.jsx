...
const emptyItem = { name: '', displayOrder: 0, isActive: true };
...
function CategoryFormFields({ data, setData, idPrefix }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <label className="block text-[0.65rem] text-eco-gold uppercase tracking-wider mb-1">Name</label>
        <input value={data.name} onChange={e => setData({ ...data, name: e.target.value })} className={inputClass} placeholder="Fresh Vegetables" />
      </div>
      <div>
...
export default function AdminCategories() {
  ...
  async function load() {
    try { setItems(await getAdminCategories()); } catch (err) { console.error(err); }
  ...
  const handleDelete = async (id, name) => {
    if (!confirm(`Delete category "${name}"?\n\nProducts that use this category will keep showing the old name until you edit them.`)) return;
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl text-eco-cream font-bold">Categories</h1>
          <p className="text-eco-cream/40 text-sm">Manage product categories. Renaming a category does not auto-update existing products.</p>
        </div>
        {!creating && (
      ...
      {creating && (
        <div className="bg-white/[0.03] border border-eco-gold/15 rounded-xl p-5 mb-4">
          <div className="text-sm text-eco-gold font-medium mb-3">New Category</div>
          <CategoryFormFields data={form} setData={setForm} idPrefix="create" />
      ...
      <div className="grid gap-3">
        {items.map(item => (
          <div key={item.id} className="bg-white/[0.03] border border-eco-gold/[0.08] rounded-xl p-5">
            {editing?.id === item.id ? (
              <div>
                <CategoryFormFields data={editing} setData={setEditing} idPrefix={`edit-${item.id}`} />
                <div className="flex gap-2 mt-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-eco-cream font-medium">{item.name}</div>
                    <div className="text-eco-cream/40 text-xs mt-0.5">Order: {item.displayOrder}{!item.isActive && <span className="text-red-400/60 ml-1">&middot; Inactive</span>}</div>
                  </div>
                </div>
